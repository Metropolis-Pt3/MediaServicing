...
.PARAMETERS
  ...
  .\WindowsIsoMediaServicing-1.1.32.ps1
...
if (Test-Path $ImagePath\install.wim) {
    ...
    Write-Host "File does NOT exist. Copying file..." -ForegroundColor Yellow
    Copy-Item -Path $InstWimPath -Destination $ImagePath -Force
}
...
if (Test-Path $Path) {
    ...
    Write-Output "Deleted: $Path" -ForegroundColor Green
} else {
    Write-Output "File not found: $Path" -ForegroundColor Red
}
...
Write-Host "Updated .Iso available. Located - C:\Temp\Virtual\Media\Win11_25H2_English_x64-New.iso" -ForegroundColor Green
}
