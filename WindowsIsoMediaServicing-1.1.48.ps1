...
.PARAMETERS
  ...
  .\WindowsIsoMediaServicing-1.1.48.ps1
...
.NOTES/REFERENCES
  ...
  7.18.2026 - Updated logical work flows. v1.1.48
...
$Temp = "C:\Temp"
$Utilities = "C:\Temp\Utilities"
$Virtual = "C:\Temp\Virtual"
$Media = "C:\Temp\Virtual\Media"

# PREPARE FOR WINDOWS MEDIA
...
$UtilitiesPath = "C:\Temp\Utilities"
$AdkPath = "C:\Temp\Utilities\Adk"
...
if($Folders -eq $True)
{
    ...
    $UtilitiesDir = Split-Path $UtilitiesPath
    if (-not (Test-Path $UtilitiesDir)) {
        New-Item -ItemType Directory -Path $UtilitiesDir -Force | Out-Null
    }
    
    $AdkDir = Split-Path $AdkPath
    if (-not (Test-Path $AdkDir)) {
        New-Item -ItemType Directory -Path $AdkDir -Force | Out-Null
    }
}
...
while ($true) {
    ...
    Write-Host "Downloading Microsoft ADK from microsoft.com..." -ForegroundColor Yellow
    Invoke-WebRequest -Uri $AdkDownloadUrl -OutFile "$AdkPath\adksetup.exe"
    ...
    $AdkFileExists = Get-ChildItem -Path $AdkPath -Filter $AdkFilePattern -File -ErrorAction SilentlyContinue

    if ($AdkFileExists) {
    Write-Host "Windows 11 ADK installer found" -ForegroundColor Green
    Write-Host "Installing Windows 11 ADK" -ForegroundColor Yellow
        Start-Process -FilePath "$AdkPath\setup.exe" -ArgumentList "/quiet /features OptionId.DeploymentTools" -Wait -PassThru
        Write-Host "Windows 11 ADK install complete" -ForegroundColor Yellow  
        }
...
if (Test-Path $ImagePath\install.wim) {
    ...
    Write-Host "Created Windows 11 25H2 Source..." -ForegroundColor Cyan
    }
    ...
    Write-Host "Created Windows 11 23H2 Source..." -ForegroundColor Cyan
    }
}
...
Write-Host "Copying Tools to Install.wim..." -ForegroundColor Cyan
...
Write-Host "Copy Tools to Install.wim complete..." -ForegroundColor Cyan
