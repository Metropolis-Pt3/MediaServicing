...
$AdkExists = Get-ChildItem -Path $ADK -Filter $AdkPattern -File -ErrorAction SilentlyContinue

if (-not $AdkExists) {
    Write-Host "Windows 11 ADK not found. Opening download page..." -ForegroundColor Yellow
    Start-Process $AdkDownloadUrl
}
else {
    Write-Host "Windows 11 ADK found:" -ForegroundColor Green
...
# Check if any Windows 11 ISO exists in the directory
$IsoExists = Get-ChildItem -Path $Temp -Filter $IsoPattern -File -ErrorAction SilentlyContinue
